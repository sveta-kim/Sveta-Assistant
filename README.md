...
- `context/GameDetector`가 세 가지 방법을 같이 쓴다: (1)
  `config/games_config.json`에 등록된 프로세스 이름과 일치하는지(리그
  오브 레전드, 발로란트, 마인크래프트 등 흔한 게임 몇 개를 기본으로
  넣어뒀고, 목록에 없는 게임은 사용자가 직접 추가하면 됨), (2) 설치된
  런처 라이브러리를 앱 시작 시 백그라운드 스레드에서 훑어서 실행
  파일명을 자동으로 목록에 더함 — `context/SteamLibraryScanner`(레지스트리로
  설치 경로 찾고 `libraryfolders.vdf` + `appmanifest_*.acf`를 최소한으로
  파싱, 실제 실행 파일명까진 안 알려줘서 설치 폴더를 깊이 3단계까지
  뒤짐)와 `context/EpicLibraryScanner`(`%ProgramData%\Epic\EpicGamesLauncher\Data\Manifests\*.item`
  JSON을 읽는데, 여긴 `LaunchExecutable` 필드가 실제 실행 파일 경로를
  직접 알려줘서 스팀과 달리 폴더를 뒤질 필요가 없음) 둘 다. 스팀은 이
  컴퓨터에서 실제로 106개를 찾아서 확인했다(처음엔 `.NET` 재배포 설치
  파일 같은 게 섞여 들어와서 필터를 더 보강함). 에픽은 이 컴퓨터에
  런처만 깔려있고 실제 설치된 게임이 없어서(매니페스트 폴더 자체가
  비어있음) 실제 게임으로는 확인 못 했고, 대신 알려진 매니페스트
  포맷대로 가짜 파일 3개(정상 게임 1개, `LaunchExecutable`이 빈
  DLC 항목 1개, 깨진 JSON 1개)를 만들어서 파싱 로직만 독립적으로
  검증했다 — 정상 항목만 정확히 걸러지고 나머지 둘은 크래시 없이
  건너뛰는 것 확인. 실제 에픽 게임으로 검증은 아직 못 했으니 안 맞는
  부분 있으면 알려주면 좋겠다. 어느 쪽이든 서버나 크라우드소싱
  데이터베이스가 아니라 사용자 컴퓨터에 실제로 설치된 것만 읽으니
  라이선스 문제는 없다. UI 스레드를 안 막으려고 백그라운드에서
  돌고(`ContextEngine`의 다른 백그라운드 읽기들과 같은 패턴), 스캔이
  끝나기 전에 `GameDetector`가 먼저 파괴돼도 안전하도록 `shared_ptr` +
  `mutex`로 공유 상태를 따로 뗐다(`ContextEngine::Create`가 트래커
  콜백에 쓰는 것과 같은 방식), (3) 위 두 방법으로도 못 찾는 게임을 위해, 창이 모니터
  전체(작업표시줄 영역 포함)를 덮으면 전체화면/전체화면형 무테두리로
  간주한다 — 원래는 `WS_CAPTION` 없음도 같이 요구했는데, 실제 게임
  (HELLDIVERS 2)에서 시각적으론 전체화면인데도 그 스타일 비트가 남아있어서
  이 조건 때문에 계속 놓치는 걸 확인하고 뺐다. 이 앱이 아직 DPI 인식
  매니페스트가 없어서(README 하단 참고) 좌표가 실제 창과 몇 픽셀
  어긋날 수 있어 여유(8px)도 뒀다. 일반 창을 최대화해도 작업표시줄
  영역까지는 안 덮으니 오탐은 거의 안 남(작업표시줄을 자동 숨김으로
  해둔 경우는 예외)
...
**게임 실시간 훈수(리그 오브 레전드 한정)**도 붙었다. 게임 감지가
"무슨 게임인지"는 알아도 "지금 라운드에 뭐가 좋을지" 같은 실시간
상황까진 몰랐는데, 이건 게임마다 사정이 완전히 다르다는 걸 확인하고
범위를 명확히 좁혔다:

- 레인보우 식스 시즈는 라이엇처럼 실시간 매치 데이터를 밖으로 주는
  공식 API가 없다. 화면을 OCR로 읽는 방법은 기술적으론 가능하지만
  패치마다 깨지는 큰 유지보수 부담이라 Vision과 함께 다음으로
  미뤘고, 게임 메모리를 직접 읽는 방법은 BattlEye 안티치트 정책
  위반(치팅 툴로 오인되어 계정 정지 위험)이라 아예 시도하지 않기로
  했다. 그래서 R6S는 그냥 채팅으로 물어보는 것 이상은 못 한다
- 반면 리그 오브 레전드는 라이엇 클라이언트 자체가 매치 진행 중에만
  로컬로 여는 공식 API가 있다(`https://127.0.0.1:2999/liveclientdata/*`,
  인증 없음, 자체 서명 인증서). `context/LeagueLiveClient`가 이걸
  읽어서 진행 시간, 내 챔피언/레벨/골드/아이템, 양 팀 전원의
  챔피언·킬/데스/어시스트·CS를 가져오고, 채팅 요청마다(`MainWindow::SendChatRequestAsync`)
  시스템 메시지로 끼워 넣는다 — 사용자가 "지금 뭐 사야 돼?" 하고
  물으면 실제 골드/아이템/상대 조합을 보고 답할 수 있다
- 실제 버그 하나 발견: 이 API에 아무것도 안 떠 있을 때(게임 중이
  아닐 때) WinHTTP 요청이 실패하기까지 생각보다 오래 걸려서(관찰상
  ~2초 — 닫힌 로컬 포트치고 이례적으로 느림), 리그 오브 레전드를
  안 하고 있어도 채팅을 보낼 때마다 2초씩 늦어지는 문제가 있었다.
  포그라운드 프로세스명이 실제로 리그 오브 레전드일 때만 이 API를
  두드리도록 고쳐서(`IsForegroundProcessLeagueOfLegends`), 관련
  없는 경우엔 네트워크 요청 자체를 안 하게 했다 — 수정 전후로 직접
  시간을 재서 확인(전: Thinking → POST 로그 사이 약 2.1초 / 후:
  약 11ms)
- 이 개발 머신엔 실행 중인 리그 오브 레전드 매치가 없어서 실제
  API로는 검증 못 했다. 대신 (a) 공식 문서에 있는 실제 응답 형태를
  그대로 본떠서 최신 클라이언트의 Riot ID 필드(`riotIdGameName`/`riotIdTagLine`)와
  구버전의 `summonerName` 필드 둘 다, 내 팀이 ORDER/CHAOS 어느
  쪽이든, 내 캐릭터를 못 찾는 경우(우리 팀을 잘못 단정 짓지 않는지)까지
  4가지 시나리오로 파싱 로직만 독립 컴파일해서 검증했고, (b)
  API가 응답하지 않을 때(현재 상태) 정말 빠르게 포기하는지도 직접
  시간을 재서 확인했다. 실제 매치로는 아직 검증 못 했으니 필드명이
  실제와 다르면 알려달라고 부탁해둔 상태
